...
if(isset($_GET['matchId'])) {
  $loadedMatch = $matchStorage->findById($_GET['matchId']);

  $_POST['home'] = $loadedMatch['home'];
  $_POST['homeScore'] = $loadedMatch['homeScore'];
  $_POST['away'] = $loadedMatch['away'];
  $_POST['awayScore'] = $loadedMatch['awayScore'];
  $_POST['date'] = $loadedMatch['date'];
  $_SESSION['matchId'] = $loadedMatch['id'];

  $_SESSION['fromTeam'] = $_GET['fromTeam'] ?? null;
  ...
  if(validate($_POST, $data, $errors)) {
      ...
      $match['id'] = $_SESSION['matchId'];
      
      $matchStorage->update($match['id'], $match);

      redirect(isset($_SESSION['fromTeam']) ? "teamDetails.php?id=".$_SESSION['fromTeam'] : 'index.php');
  }
}
?>
...
<head>
  ...
  <title>ELStadion - Meccsváltoztatás</title>
</head>
<body>
  <?php headerHTML(); ?>
          ...
          <div class="card">
             <div class="card-body">
                <div class="text-center m-auto">
                   <h4 class="text-center">Mérkőzés adatainak változtatása</h4>
                </div>

                <form action="editMatch.php" method="post" novalidate>
                  <div class="form-group mb-2">
                   ...
                   <div class="form-group mb-2">
                      ...
                      <select name="home" id="home">
                        <?php foreach($teams as $team) : ?>
                          <option value="<?= $team['id'] ?>" <?= $team['id'] === $_POST['home'] ? 'selected' : '' ?>><?= $team['name'] ?> (<?=$team['city']?>)</option>
                        <?php endforeach ?>
                      </select>
                   ...
                   <div class="form-group mb-2">
                      ...
                      <input type="number" name="homeScore" placeholder="<?= $_POST['homeScore'] ?? '' ?>" min="0" class="form-control" value="<?= $_POST['homeScore'] ?? '' ?>">
                   ...
                   <div class="form-group mb-2">
                      ...
                      <select name="away" id="away">
                        <?php foreach($teams as $team) : ?>
                          <option value="<?= $team['id'] ?>" <?= $team['id'] === $_POST['away'] ? 'selected' : '' ?>><?= $team['name'] ?> (<?=$team['city']?>)</option>
                        <?php endforeach ?>
                      </select>
                   ...
                   <div class="form-group mb-2">
                      ...
                      <input type="number" name="awayScore" placeholder="<?= $_POST['awayScore'] ?? '' ?>" min="0" class="form-control" value="<?= $_POST['awayScore'] ?? '' ?>">
                   ...
                   <div class="form-group mb-0 text-center">
                      <a href="<?=isset($_SESSION['fromTeam']) ? "teamDetails.php?id=".$_SESSION['fromTeam'] : 'index.php'?>"><span class="btn btn-primary btn-block">Vissza</span></a>
                      <a href="deleteMatch.php?matchId=<?=$_SESSION['matchId']?>&fromTeam=<?=isset($_SESSION['fromTeam']) ? $_SESSION['fromTeam'] : null ?>"><span class="btn btn-danger btn-block">Meccs törlése</span></a>
                      <button class="btn btn-success btn-block" type="submit" name="submit">Meccs frissítése</button>
                   </div>
